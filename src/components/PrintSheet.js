...
const convertToUnit = (str, toUnit = 'mm', map = unitMap) => {
  const supportedUnits = Object.keys(map);
  const reg = new RegExp(`(.*)(${supportedUnits.join('|')})`);
  const [match, val, unit] = str.match(reg);
    ...
    return (<div className={classes.root}>
      ...
      { range(pageCount).map( pageIndex => (
        <Paper key={pageIndex} elevation={0} className={classes.page} style={{
          ...
          gridTemplateColumns: `repeat(${itemsPerPageWith}, ${item.width}${UNIT})`,
        }}>
